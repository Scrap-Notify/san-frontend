  ...
  return (
    <section className="relative grid min-h-[min(70vh,34rem)] w-full min-w-0 overflow-hidden rounded-leaf bg-surface-low px-6 py-12 text-center sm:px-8">
      <div className="pointer-events-none absolute inset-0 opacity-80">
        ...
        <div className="absolute right-8 top-8 hidden h-48 w-48 rounded-leaf border border-white/5 bg-misty-teal/20 opacity-30 sm:block" />
      </div>
          ...
          <div className="flex flex-wrap items-center justify-center gap-3 pt-2">
            ...
            {secondaryAction ? (
              <button
                ...
                className="inline-flex min-h-11 items-center justify-center rounded-leaf border border-white/10 bg-white/5 px-6 text-body-sm-bold font-bold text-text-primary transition hover:bg-white/10"
              >
    ...
    <article className="group relative flex h-[280px] w-[min(88vw,24rem)] min-w-0 shrink-0 snap-start flex-col justify-between rounded-tl-[32px] rounded-br-[32px] rounded-tr-2xl rounded-bl-2xl bg-[#131718] p-6 shadow-md md:w-[calc((100%-24px)/2)] xl:w-[calc((100%-48px)/3)]">
      <div>
        ...
        <div className="flex items-start gap-3">
          <div className="mt-1 flex h-10 w-10 shrink-0 items-center justify-center rounded-tl-[18px] rounded-br-[18px] rounded-tr-md rounded-bl-md bg-white/5 text-white/50">
      ...
      <div className="flex justify-end">
        {primaryAction ? (
          ...
          <div className="flex h-11 w-11 items-center justify-center rounded-tl-[20px] rounded-br-[20px] rounded-tr-md rounded-bl-md bg-white/5 text-white/60">
