    ...
    >
      <div className="flex min-w-0 items-center gap-4">
        ...
        <div className="min-w-0">
          <p className="truncate text-body-main text-[#e0e3e7]">{toast.title}</p>
          {toast.description ? (
      ...
      <button
        type="button"
        onClick={onDismiss}
        className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full text-text-secondary/60 transition hover:bg-white/5 hover:text-text-primary"
        aria-label="알림 닫기"
      >
...
function ToastIcon({ type }: { type: ToastType }) {
  ...
  if (type === 'success') {
    return (
      <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-leaf bg-primary-signal text-[#007255]">
