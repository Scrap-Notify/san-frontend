  ...
  return (
    <nav className="w-full min-w-0 rounded-3xl border border-white/5 bg-black/35 px-4 py-3 shadow-[0_18px_50px_rgba(0,0,0,0.18)] backdrop-blur-xl md:rounded-full md:px-5">
      <div className="flex min-w-0 items-center justify-between gap-4">
        ...
        <button
          type="button"
          className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full border border-white/5 bg-[#1b2023] text-[#b9cbc1] transition hover:border-primary-signal/30 hover:text-white md:hidden"
...
function SearchForm({
  ...
  return (
    <form
      ...
      role="search"
    >
      <div className="flex min-h-9 min-w-0 items-center gap-2 rounded-full border border-white/5 bg-[#1b2023] px-3 py-1.5 text-[#b9cbc1] transition focus-within:border-primary-signal/40 focus-within:text-white md:w-[min(22vw,18rem)] md:min-w-44">
      ...
      <label className="flex min-h-9 min-w-0 items-center gap-2 rounded-full border border-white/5 bg-[#1b2023] px-3 py-1.5 text-xs font-medium text-[#b9cbc1] transition focus-within:border-primary-signal/40 focus-within:text-white">
      ...
      <button
        type="submit"
        className="flex min-h-9 items-center justify-center rounded-full bg-primary-signal px-4 py-1.5 text-xs font-bold text-[#101417] transition hover:bg-primary-signal-hover"
      >
        Search
      </button>
    </form>
...
function TagPicker({
  ...
  return (
    <div className="group relative">
      <button
        type="button"
        className="flex min-h-9 w-full items-center justify-center gap-2 rounded-full border border-white/5 bg-[#1b2023] px-3 py-1.5 text-xs font-medium text-[#b9cbc1] transition hover:border-primary-signal/30 hover:text-white md:w-auto"
      >
...
function DesktopActions({
  ...
  return (
    <>
      <Link
        to="/til"
        className="flex min-h-9 min-w-0 items-center justify-center gap-2 rounded-full bg-primary-signal px-5 py-1.5 text-sm font-bold text-[#101417] shadow-[0_0_24px_rgba(0,255,194,0.16)] transition hover:bg-primary-signal-hover"
      >
      ...
      <Link
        ...
        className={[
          'transition',
          locationPath.startsWith('/settings') ? 'text-primary-signal' : 'text-[#b9cbc1] hover:text-white',
        ].join(' ')}
        aria-label="Settings"
      >
        <span className="flex h-9 w-9 items-center justify-center rounded-full border border-white/5 bg-[#1b2023] transition hover:border-primary-signal/30">
      ...
      <Link
        to="/account"
        className="h-9 w-9 shrink-0 overflow-hidden rounded-full bg-[#1e5056] ring-1 ring-white/10 transition hover:ring-primary-signal/40"
        aria-label="Account"
      >
      ...
      <button
        type="button"
        onClick={onLogout}
        className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full border border-white/5 bg-[#1b2023] text-[#b9cbc1] transition hover:border-primary-signal/30 hover:text-white"
        aria-label="Logout"
      >
        <LogOut className="h-4 w-4" />
      </button>
